...
function normalizeInput(type, raw) {
  ...
  if (type === 'monster') {
    for (const [collection, targetKind] of Object.entries(monsterKinds)) {
      ...
      for (const component of { movements, attacks, abilities, skills, traits }[collection]) {
        monsterComponents.push({ kind, componentId: component.id, visibility: component.visibility, data: component.data });
      }
    }
  }
...
async function replaceSimpleChildren(tx, entityId, type, split) {
  ...
  if (split.fields.length) {
    await tx.narrativeField.createMany({
      data: split.fields.map((field) => ({ entityId, key: field.key, value: field.value, visibility: field.visibility }))
    });
  }
  ...
  return prisma.$transaction(async (tx) => {
    const entity = await tx.entity.create({
      data: {
        ...
        baseVisibility: input.baseVisibility ?? 'public',
        data: split.data,
...
export async function updateEntity({ campaignId, type, domainId, input, version, actorUserId, source }) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const result = await tx.entity.updateMany({
      ...
      data: {
        ...
        name: parsed.name,
        active: parsed.active,
        baseVisibility: input.baseVisibility ?? current.baseVisibility,
        data: split.data,
...
export async function deleteEntity({ campaignId, type, domainId, actorUserId }) {
  return prisma.$transaction(async (tx) => {
    ...
    await tx.entity.update({ where: { id: current.id }, data: { deletedAt: new Date(), version: { increment: 1 } } });
    return true;
  });
}
...
function accessFor({ baseVisibility, request, context, grants, targetKind, targetKey }) {
  const relevant = grants.filter((grant) => grant.targetKind === targetKind && grant.targetKey === targetKey);
...
function filterMonsterSheet({ sheet = {}, request, context, grants }) {
  ...
  for (const key of ['nd', 'type', 'subtype', 'size']) {
    if (sheet[key] != null && allowed(key)) output[key] = sheet[key];
  }
  for (const moduleName of ['combat', 'resources', 'resistances', 'attributes']) {
    ...
    for (const [key, value] of Object.entries(moduleValue)) {
      if (allowed(`${moduleName}.${key}`, visibilityMap[moduleName] ?? 'public')) filtered[key] = value;
    }
    ...
    : await prisma.grant.findMany({
        where: {
          ...
          OR: [
            ...
            ...(context.groups.length ? [{ subjectType: 'group', subjectId: { in: context.groups } }] : [])
          ]
        }
      });
    ...
    const target = await prisma.entity.findUnique({
      where: {
        campaignId_type_domainId: {
          ...
          domainId: reference.targetDomainId
        }
      },
      include: { fields: true, references: true, locationConnections: true, monsterComponents: true, questObjectives: true, questRewards: true }
    });
    if (!target || target.deletedAt) {
      if (context.gm) references.push({ type: reference.targetType, id: reference.targetDomainId, role: reference.role, chance: reference.chance, broken: true });
      continue;
    }
    const targetGate = await serializeEntityGateOnly(target, request);
    if (targetGate) references.push({ type: reference.targetType, id: reference.targetDomainId, name: target.name, role: reference.role, chance: reference.chance });
  }
  ...
  if (entity.type === 'monster') {
    ...
    for (const kind of ['movement', 'attack', 'ability', 'skill', 'trait']) {
      ...
      result[outputKey] = entity.monsterComponents
        ...
        .map((component) => ({ id: component.componentId, data: component.data, ...(context.gm ? { visibility: component.visibility } : {}) }));
    }
  }
  ...
  if (entity.type === 'quest') {
    result.objectives = entity.questObjectives
      ...
      .map((objective) => ({
        ...
        requiredQuantity: objective.requiredQuantity,
        optional: objective.optional,
        ...(context.gm ? { secret: objective.secret, visibility: objective.visibility, playerEditable: objective.playerEditable } : {}),
        ...(objective.targetType ? { target: { type: objective.targetType, id: objective.targetDomainId } } : {}),
        dependsOn: context.gm ? objective.dependsOn : undefined
      }));
    if (context.gm) result.rewards = entity.questRewards.map((reward) => reward.data);
  }

  if (context.gm) {
    result._technical = { entityPk: entity.id, baseVisibility: entity.baseVisibility, source: entity.source };
  }
  return result;
}
...
export async function listEntitiesForRequest({ request, type, page = 1, pageSize = 50, search = '', sort = 'name' }) {
  const where = {
    campaignId: request.campaign.id,
    type,
    deletedAt: null,
...
export async function searchForRequest({ request, query, limit = 30 }) {
  const rows = await prisma.entity.findMany({
    where: { campaignId: request.campaign.id, deletedAt: null, name: { contains: query, mode: 'insensitive' } },
    include: includeAll,
    orderBy: { name: 'asc' },
    take: Math.min(limit * 3, 100)
  });
  const visible = (await Promise.all(rows.map((row) => serializeEntityForRequest(row, request)))).filter(Boolean);
  return visible.slice(0, limit).map((row) => ({ type: row.entityType, id: row.id, name: row.name }));
}
...
export async function softRemoveImportedEntityTx({ tx, campaignId, type, domainId, actorUserId, source }) {
  const current = await tx.entity.findUnique({
    where: { campaignId_type_domainId: { campaignId, type, domainId } }
  });
  if (!current) return null;
...
export function entityRecordToAuthoritative(entity) {
  const result = {
    ...
    name: entity.name,
    active: entity.active,
    fields: (entity.fields ?? []).map((field) => ({ key: field.key, value: field.value, visibility: field.visibility })),
  ...
  if (entity.type === 'monster') {
    ...
    for (const kind of ['movement', 'attack', 'ability', 'skill', 'trait']) {
      result[`${kind}s`] = (entity.monsterComponents ?? [])
        .filter((component) => component.kind === kind)
        .map((component) => ({ id: component.componentId, visibility: component.visibility, data: component.data }));
    }
  }
  if (entity.type === 'quest') {
